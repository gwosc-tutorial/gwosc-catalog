...
class ParameterValue:
    ...
    upper_error: float
        ...
        OK to use different uncertainty definition if noted in
        documentation.

    lower_error: float
        Size of the lower error-bar of the 90% credible region.
        OK to use a different uncertainty definition if noted in
        documentation.
    ...
    is_lower_bound: bool
        True if this value is an upper bound, False otherwise.
        Defaults to False if omitted.
        Setting this to True displays a greater-than sign before the
        value.

    decimal_places: int
        Number of decimal places of the best value to display, must be
        >= 0. Displayed values will be rounded to this number of decimal
        places.
    ...
    def __post_init__(self):
        ...
        if self.parameter_name == "luminosity_distance" and self.unit != "Mpc":
            raise ValueError(
                "luminosity_distance parameter needs to have Mpc `unit`.")
        ...
        if self.is_upper_bound and self.is_lower_bound:
            raise ValueError(
                "Both `is_upper_bound` and `is_lower_bound` set to True.")
...
class Link:
    ...
    For a link to posterior samples, use ``ParameterSet.data_url``
    instead.
    """
...
class ParameterSet:
    ...
    data_url: str, url
        This should point to the source data for the analysis of each
        event. It typically points to a posterior sample file if
        available.
...
class Event:
    ...
    gracedb_id: str or None
        ...
        This is an optional key but we recommend adding it whenever
        possible.
    ...
    def __post_init__(self):
        ...
        if self.detectors is not None:
            for detector in self.detectors:
                if detector not in ["H1", "L1", "V1", "K1", "G1"]:
                    raise ValueError(
                        f"Unrecognized detector short name: {detector}")
        ...
        n_preferred = np.count_nonzero(
            [pe_set.is_preferred for pe_set in self.pe_sets])
        if n_preferred != 1:
            raise ValueError("Exactly one of the `pe_sets` should be "
                             f"preferred, got {n_preferred}.")
    ...
    def from_json(cls, event: dict):
        ...
        return Event(
            **e,
            ...
            pe_sets=[ParameterSet.from_json(pe_set)
                     for pe_set in pe_sets] or None,
        )
...
class Catalog:
    ...
    release_date: str
        The date of the public release of the data in the format
        YYYY-MM-DD.
    ...
    def __post_init__(self):
        ...
        if self.schema_version != __version__:
            warnings.warn(
                f"Schema version provided {self.schema_version} is different "
                f"from current: {__version__}",
                UserWarning,
                stacklevel=1,
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="validateschema",
        description="""
            Validate the upload json schema for a GWOSC community catalog.""",
    )
